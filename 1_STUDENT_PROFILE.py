...
def ranksNresults_menu():
    ...
    if text_search:
        ...
        m1 = df["NAME"].str.contains(text_search.upper(), regex=False, na=False)
        if year_choosed == '2028':
            m2 = df["ROLL NO."].str.contains(text_search.upper(), regex=False, na=False)
            df_search = df[m1 | m2]
...
def placement_menu():
    ...
    data21 = pd.read_csv('./Extracting_Result_Data/placement_data/percentage_placed21.csv').dropna()
    placed_Data = []
...
if selected == 'PROFILE':
    ...
    if year_choosed == '2026 (NOT UPDATED YET)':
        ...
        m1 = csvdf['ROLL NO. OG'].str.contains(result_search_box.upper(), regex=False, na=False)
