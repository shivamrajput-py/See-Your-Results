...
# MAIN MENU COLUMNS FOR LOGO ANIMATION AND REAL MAIN MENU [0.4,3,0.4]
...
with mainmenu_middle:
    selected = option_menu(menu_title=None, options=['PROFILE', 'RANKS', 'PLACEMENTS', 'STUDY', 'SGPA', 'ABOUT'],
                           default_index=0,
                           icons=['person-vcard', 'bar-chart-line', 'clipboard-data', 'journals', 'calculator-fill',
                                  'info-square'],
        ...
        if (len(df_final) > 1 or len(df_final) < 1) and not other:
            ...
            with l_sec2:
                ...
                df = pd.DataFrame({
                    ...
                    'CGPA': [brnch_cg['Highest'].values[i], brnch_cg['Average'].values[i], brnch_cg['Your'].values[i],
                             brnch_cg['Max Appeared'].values[i], brnch_cg['Lowest'].values[i]]})

                df.reset_index(drop=True)
                df.set_index('Branch Stats', inplace=True)

                st.plotly_chart(px.bar(df, title=f"{stud_branch}'27 CGPA Distribution", range_y=[0, 10], text_auto='',
                                       width=430).update_layout({'dragmode': False}),
            ...
            with r_sec2:
                ...
                df = pd.DataFrame({
                    ...
                    'CGPA': [brnch_cg['Highest'].values[0], brnch_cg['Average'].values[0], brnch_cg['Your'].values[0],
                             brnch_cg['Max Appeared'].values[0], brnch_cg['Lowest'].values[0]]})

                df.reset_index(drop=True)
                df.set_index('University Stats', inplace=True)

                st.plotly_chart(px.bar(df, title=f'University CGPA Distribution', range_y=[0, 10], text_auto='',
                                       width=430).update_layout({'dragmode': False}),
                                config={"modeBarButtonsToRemove": ['lasso2d', 'autoScale2d', 'zoomOut2d', 'select2d']})
            ...
            df = pd.read_csv(f'./Extracting_Result_Data/ranked_results_csv/1_{stud_branch}_ranked_results.csv',
                             dtype=str, index_col=None).fillna("")
            ...
            avg_placement_barc = px.bar(df, title=f"Average Package Trend (IN LPA)", text_auto='').update_layout(
                {'dragmode': False})
            ...
            max_placement_barc = px.bar(df, title=f"Highest Package Trend (IN LPA)", text_auto='').update_layout(
                {'dragmode': False})
        ...
        if (len(df_final) > 1 or len(df_final) < 1) and not other:
            ...
            with l_c:
                ...
                l2.metric(label="2ND SEM CG", value=df_final['SEM2'].values[0],
                          delta=round(float(df_final['SEM2'].values[0]) - float(df_final['SEM1'].values[0]), 2))
                l3.metric(label="3RD SEM CG", value=df_final['SEM3'].values[0],
                          delta=round(float(df_final['SEM3'].values[0]) - float(df_final['SEM2'].values[0]), 2))
            ...
            with left_c:
                ...
                brnch_cg = pd.read_csv(
                    f'./Extracting_Result_Data/ranked_results_csv/26_{stud_branch}_ranked_results.csv', index_col=False)
                ...
                st.plotly_chart(
                    px.bar(df, title=f"{stud_branch}'26 CGPA Distribution 2026", text_auto='').update_layout(
            ...
            with right_c:
                ...
                st.plotly_chart(px.bar(df, title=f'University CGPA Distribution 2026', text_auto='').update_layout(
                    {'dragmode': False}), use_container_width=True, config={
